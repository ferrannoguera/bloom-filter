...
int randomentre2500i7500(){
	return rand() % 5000 + 2500;
}

int randomentre0i10000(){
    return rand() % 10000;
}
...
int main () {
    srand(time(0));
    string s;
    int a_comp = randomentre2500i7500();
    vector<string> vs(0);
    while (cin >> s) vs.push_back(s);
    for(int i=0 ;i<6;i++){
		string test="test", txt=".txt",carpeta="tests/deumil/";
		...
		for(int j=0;j<a_comp;j++){
			outfile << vs[randomentre0i10000()] << endl;
		}
        outfile.close();
    }   
}
